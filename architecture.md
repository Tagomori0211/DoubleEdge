...
「クオータ制限下でもLLMエージェントの実行品質を落とさずにするにはどうすればいいか？」という課題を解決するエージェントループとして実装したもの。

- DS（DeepSeek V4 Pro）= 低コスト・オープンウェイト・コントロールプレーンに最適。
- AG×3（Antigravity CLI）= 並列実行部隊。Gemini 3.5 Flash の並列速度を活用。
