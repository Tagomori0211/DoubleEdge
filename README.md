...
- **依存ツール**: `tmux`, `git`, `curl`, `cline`, `agy`, `claude`
  → `install.sh` が [`requirements.txt`](./requirements.txt) を参照して不足分を自動導入します
...
`setup-doubleedge.sh` が起動時に `.env` を読み込み、各 pane（特に pane 0: DS / cline）へ
環境変数として継承させます。`.env` は `.gitignore` 済みです。
...
```
DoubleEdge/
├── install.sh                # 依存の自動導入 + .env ひな型作成
├── requirements.txt          # 必要ツール一覧（install.sh が解析）
